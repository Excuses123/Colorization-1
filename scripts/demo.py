...
def parse_argument():
    ...
    """
    ap = ArgumentParser()
    ap.add_argument("-t", "--type", help="load weights or model")
    args = vars(ap.parse_args())
    if args['type'] == 'weights':
        model_ = build_model()
        model_.load_weights(glob.glob("../models/*weights.h5")[0])
    else:
        model_ = load_model(glob.glob("../models/*model.h5")[0])
    return model_
...
def get_images():
    ...
    :return:
    """
    filenames = glob.glob("../data/images/valid/*")
    images_bgr = []
    images_gray = []
...
def predict():
    ...
    :return:
    """
    model = parse_argument()
    print(model.summary())

    h, w = IMG_HEIGHT // 4, IMG_WIDTH // 4
    q_ab = np.load("../data/params/pts_in_hull.npy")
    number_q = q_ab.shape[0]
    nn = NearestNeighbors(n_neighbors=NEIGHBOUR_NUM, algorithm='ball_tree').fit(q_ab)

    images_bgr, images_gray = get_images()
    for i in range(images_bgr.shape[0]):
        ...
        cv2.imwrite('../test_result/{}_gray.png'.format(i), images_gray[i])
        cv2.imwrite('../test_result/{}_raw.png'.format(i), images_bgr[i])
        cv2.imwrite('../test_result/{}_pred.png'.format(i), out_bgr)
